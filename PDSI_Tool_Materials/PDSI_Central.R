...
PDSI_Central<- function(Z_all,count_loc,beg_row_col,end_row_col,lat_col,yr_col,PET){
...
PET_col <- reshape(t(PET),length(PET)*12,1)
table <- vector()
l <- vector()

for (j in 1:count_loc){
    ...
    BT <- rep(0, l[j])# BT is the backtracking variable, and is
    ...
    for (k in 1:l[j]){
        ...
        if (Pe == 100 || Pe == 0){# No abatement underway
            if (abs(X3) <= 0.5){# Drought or wet spell ends
                ...
                PX3[k] <- 0# PX3 is the preliminary X3 value and is used
                            # in operational calculations.
                lst <- Main(Z,k,PV,PPe,X1,X2,PX1,PX2,PX3,X,BT)
                sapply(toString(c(PX1,PX2,PX3,X,BT)), function(x) {id=1; assign(x, lst[[id]]); id = id+1})
            } else if (X3 > 0.5){# Wet spell underway
                if (Z[k] >= 0.15){# Wet spell intensifies
                  lst <- Between0s(k,Z,X3,PX1,PX2,PX3,PPe,BT,X)
                  sapply(toString(c(PV,PX1,PX2,PX3,PPe,X,BT)), function(x) {id=1; assign(x, lst[[id]]); id = id+1})
                } else {# Wet spell starts to abate, and it may end.
                  lst <- Function_Ud(k,Z,V, Pe,PPe,PX1,PX2, PX3,X1,X2,X3,X,BT)
                  sapply(toString(c(Ud,Q,PV,PPe,PX1,PX2,PX3,X,BT)), function(x) {id=1; assign(x, lst[[id]]); id = id+1})
                }
            } else if (X3 < -0.5){# Drought underway
                if (Z[k] <= -0.15){# Drought intensifies
                    lst <- Between0s(k,Z,X3,PX1, PX2,PX3,PPe,BT,X)
                    sapply(toString(c(PV,PX1,PX2,PX3,PPe,X,BT)), function(x) {id=1; assign(x, lst[[id]]); id = id+1})
                } else {# Drought starts to abate, and it may end.
                    lst <- Function_Uw(k,Z,V, Pe,PPe,PX1,PX2, PX3,X1,X2,X3,X,BT)
                    sapply(toString(c(Uw,Q,PV,PPe,PX1,PX2,PX3,X,BT)), function(x) {id=1; assign(x, lst[[id]]); id = id+1})
                }
            }
        ...
        if (k > 1){
            if (PX3(k-1) == 0 && BT(k-1) == 0){
                for (cc in k -1 : -1 : 1){
                    if (BT[cc] != 0){# Backtracking continues in a
                                  # backstepping procedure up through the
                                  # first month where BT is not equal to
                                  # zero.
                        r <- cc + 1# r is the row number up through which
    ...
    table_loc <- c(lat_loc, yr_loc, PET_loc, Z_loc, t(PPe), t(PX1), t(PX2),t(PX3), t(X), PDSI, t(PHDI))
    # Vertically concatenate output tables for all locations.
    table <- c(table, table_loc)
